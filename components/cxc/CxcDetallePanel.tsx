// components/cxc/CxcDetallePanel.tsx
...
const DONE_ACCESSORY_ID = "cxcPanelDone";
...
async function downloadToCache(remoteUrl: string, mimeHint?: string) {
  ...
  const ext =
    (extFromHint || "").toLowerCase() ||
    (["jpg", "jpeg", "png", "webp", "heic", "heif"].includes(extFromUrl)
      ? extFromUrl === "jpeg"
        ? "jpg"
        : extFromUrl
      : "jpg");
...
function CxcDetallePanelWithParams({ fallbackVentaId }: { fallbackVentaId: number | null }) {
  const { ventaId } = useLocalSearchParams<{ ventaId?: string }>();
  const idFromParams = ventaId ? Number(ventaId) : NaN;
  const resolvedId =
    Number.isFinite(idFromParams) && idFromParams > 0 ? idFromParams : fallbackVentaId;
  return <CxcDetallePanelContent ventaIdProp={resolvedId} embedded={false} />;
}
...
function CxcDetallePanelContent({
  ...
  useFocusEffect(
    useCallback(() => {
      void refreshRole("focus:cxc-detalle-panel");
    }, [refreshRole])
  );
  ...
  const guardarPago = async () => {
    if (!row || savingPago) return;
    const montoRaw = String(pagoMonto ?? "").trim();
    const monto = safeNumber(pagoMonto);
    if (!montoRaw) { Alert.alert("Falta monto", "El monto es obligatorio."); return; }
    ...
    const isPendiente = (facturasPendientes ?? []).some(
      (ff: any) => Number(ff?.id) === Number(pagoFacturaId)
    );
    if (!isPendiente) {
    ...
    try {
      ...
      if (error) throw error;
      Alert.alert("Pago reportado", "El pago fue enviado a aprobación del administrador.");
  ...
  const downloadViewerImage = useCallback(async () => {
    ...
    try {
      ...
      try { await MediaLibrary.createAlbumAsync("Downloads", asset, false); } catch {}
  ...
  return (
    <>
      {!embedded && (
        <Stack.Screen
          options={{ headerShown: true, title: "Detalle cuenta", headerBackTitle: "Atrás" }}
        />
      )}
                ...
                <View
                  style={[
                    styles.badgePill,
                    {
                      ...
                      borderColor:
                        alphaColor(saldoNum <= 0 ? C.ok : C.warn, isDark ? 0.32 : 0.22) ||
                        C.border,
                    },
                  ]}
                >
            ...
            {lineas.length === 0 ? (
              ...
              >
                ...
                >
                  ...
                  <Text
                    style={[styles.td, { color: C.text, width: 140, textAlign: "right" }]}
                  >
                    ...
                    <View style={styles.rowBetween}>
                      ...
                      >
                        ...
                        {(() => {
                          ...
                          return monto != null ? (
                            <Text
                              style={[styles.payAmount, { color: C.text }]}
                              numberOfLines={1}
                            >
                              {fmtQ(monto)}
                            </Text>
              ...
              {saldoNum > 0 ? (
                hasPendingReport ? (
                  ...
                  >
                    ...
                    <Text
                      style={{ color: C.sub, marginTop: 4, fontWeight: "600", fontSize: 12 }}
                    >
                  ...
                  <AppButton
                    ...
                    onPress={() => {
                      const only =
                        (facturasPendientes ?? []).length === 1
                          ? Number((facturasPendientes as any[])[0]?.id)
                          : null;
                      setPagoFacturaId(
                        Number.isFinite(only as any) && (only as any) > 0 ? (only as any) : null
                      );
                      setPagoMetodo(allowedMetodosPago[0]);
                      setFacturaPickerOpen(false);
                      setPagoModal(true);
                    ...
                    variant="primary"
                    style={
                      { marginTop: 12, backgroundColor: C.primary, borderColor: C.primary } as any
                    }
                  />
                )
              ) : null}
                        ...
                        {facturaPickerOpen ? (
                          <>
                            ...
                            <Text style={[styles.modalSub, { color: C.sub }]}>
                              El pago se aplicará a la factura elegida.
                            </Text>
                            {facturasPendientes.length === 0 ? (
                              <Text
                                style={{ color: C.sub, marginTop: 12, fontWeight: "700" }}
                              >
                                No hay facturas pendientes para esta venta.
                              </Text>
                            ) : (
                              <ScrollView
                              ...
                              >
                                {facturasPendientes.map((f: any) => {
                                  const fid = Number(f?.id);
                                  const active =
                                    !!pagoFacturaId && fid === Number(pagoFacturaId);
                                  const numero =
                                      ...
                                      style={({ pressed }) => [
                                        {
                                          borderWidth: StyleSheet.hairlineWidth,
                                          borderColor: active
                                            ? alphaColor(C.primary, 0.35) || C.border
                                            : C.border,
                          ...
                          <>
                            ...
                            <Text style={[styles.modalSub, { color: C.sub }]}>
                              Saldo: {fmtQ(row?.saldo)}
                            </Text>
                            {facturasPendientes.length === 0 ? (
                              <Text
                                style={{ color: C.sub, marginTop: 10, fontWeight: "700" }}
                              >
                                No hay facturas pendientes para esta venta.
                              </Text>
                            ) : null}
                            ...
                            >
                              <Text
                                style={{
                                  color: selectedFactura ? C.text : C.sub,
                                  fontWeight: "700",
                                }}
                                numberOfLines={1}
                              >
                                {selectedFactura
                                  ? facturaLabel(selectedFactura)
                                  : "Seleccionar factura"}
                              </Text>
                            </Pressable>
                            ...
                            <Text
                              style={[styles.inputLabel, { color: C.sub, marginTop: 10 }]}
                            >
                              Método
                            </Text>
                            <View
                            ...
                            >
                              {allowedMetodosPago.map((m) => {
                                ...
                                return (
                                  ...
                                  >
                                    <Text
                                      style={{ color: C.text, fontWeight: "800", fontSize: 12 }}
                                    >
                                      {m}
                                    </Text>
                                  </Pressable>
                            ...
                            {roleNormalized !== "ADMIN" ? (
                              <Text
                                style={{ color: C.sub, marginTop: 6, fontWeight: "600", fontSize: 12 }}
                              >
                            ...
                            <View style={{ marginTop: 12 }}>
                              <Text style={[styles.inputLabel, { color: C.sub }]}>
                              ...
                              <Pressable
                                ...
                                style={({ pressed }) => {
                                  ...
                                  const primaryBorder =
                                    alphaColor(C.primary, isDark ? 0.45 : 0.35) || C.border;
                                  const primaryBg =
                                    alphaColor(C.primary, isDark ? 0.14 : 0.08) || C.mutedBg;
                                  return [
                                ...
                                <View style={styles.comprobanteCtaInner}>
                                  <View
                                    style={[
                                      ...
                                      {
                                        backgroundColor: pagoImg?.uri
                                          ? C.okBg
                                          : alphaColor(C.primary, isDark ? 0.14 : 0.08) ||
                                            C.mutedBg,
                                        borderColor:
                                          alphaColor(
                                            pagoImg?.uri ? C.ok : C.primary,
                                            isDark ? 0.32 : 0.22
                                          ) || C.border,
                                      },
                                    ]}
                                  >
                                    <Text
                                      style={[
                                        styles.comprobanteBadgeText,
                                        { color: pagoImg?.uri ? C.ok : C.primary },
                                      ]}
                                    >
                                      {pagoImg?.uri ? "OK" : "+"}
                                    </Text>
                                  ...
                                  <View style={{ flex: 1, minWidth: 0 }}>
                                    <Text
                                      style={{ color: C.text, fontWeight: "900", fontSize: 14 }}
                                      numberOfLines={1}
                                    >
                                      {pagoImg?.uri
                                        ? "Comprobante adjunto"
                                    ...
                                    >
                                      {pagoImg?.uri
                                        ...
                                        : "Requerido para habilitar Guardar"}
                                    </Text>
                                  </View>
                                  {pagoImg?.uri ? (
                                    <Image
                              ...
                              <Text
                                style={{ color: C.sub, marginTop: 8, fontWeight: "600", fontSize: 12 }}
                              >
                                {pagoImg?.uri
                                  ? "Listo. Se subirá junto con el pago."
                                  : "Debes adjuntar una imagen como comprobante."}
                              </Text>
                            </View>
            ...
            <View style={[styles.viewerBg, { backgroundColor: "rgba(0,0,0,0.8)" }]}>
              <View style={styles.viewerCard}>
                <View style={[styles.viewerTopBar, { top: Math.max(12, insets.top + 10) }]}>
                  ...
                  >
                    <Text style={styles.viewerTopBtnText}>
                      {savingDownload ? "Guardando..." : "Descargar"}
                    </Text>
                  </Pressable>
                </View>
