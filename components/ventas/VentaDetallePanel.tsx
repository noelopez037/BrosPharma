...
function VentaDetallePanelWithParams({ fallbackVentaId }: { fallbackVentaId: number }) {
  const params = useLocalSearchParams<VentaDetalleRouteParams>();
...
function VentaDetallePanelContent({ embedded, ventaIdProp, params: routeParams, onEditWeb, refreshKey }: VentaDetallePanelContentProps) {
  ...
  const ventaIdRaw = embedded ? String(ventaIdProp ?? "") : String(routeParams?.ventaId ?? "");
  const ventaIdNum = embedded ? Number(ventaIdProp) : Number(ventaIdRaw);
  const returnTo = embedded ? "" : String(routeParams?.returnTo ?? "");

  const notifKind = embedded ? "" : String(routeParams?.notif ?? "").trim().toUpperCase();
  const notifAccion = embedded ? "" : String(routeParams?.accion ?? "").trim().toUpperCase();
  const notifNota = embedded ? "" : String(routeParams?.nota ?? "").trim();
  const notifCliente = embedded ? "" : String(routeParams?.clienteNombre ?? "").trim();
  const notifVendedorCodigo = embedded ? "" : String(routeParams?.vendedorCodigo ?? "").trim();
  const shouldShowSolicitudBanner = !embedded && notifKind === "VENTA_SOLICITUD_ADMIN";
  ...
  const fetchVentaEventos = useCallback(async () => {
  ...
  const pasarEnRuta = useCallback(
    ...
    [canBodega, enRutaLoading, fetchVentaEventos, fetchVenta, isFacturado, venta]
  );
    ...
    <>
      ...
      <SafeAreaView style={[styles.safe, { backgroundColor: C.bg }]} edges={["bottom"]}>
        ...
        >
          ...
          {!Number.isFinite(ventaIdNum) || ventaIdNum <= 0 ? (
           ...
           {venta ? (
             <View style={[styles.card, { borderColor: C.border, backgroundColor: C.card }]}>
               {venta.estado === 'FACTURADO' ? (
                 ...
                 <View style={{ position: "relative", paddingRight: Platform.OS === 'web' ? 260 : 0 }}>
                   <Text style={[styles.clientName, { color: C.text, fontSize: Platform.OS === 'web' ? 18 : 14 }]} numberOfLines={1} adjustsFontSizeToFit>
                     {venta.cliente_nombre ?? clienteMini?.nombre ?? "—"}
                   </Text>
                   {clienteMini?.telefono && clienteMini.telefono.trim() !== '-' && clienteMini.telefono.trim() !== '' ? (
                     <Text style={[styles.clientNit, { color: C.sub, marginTop: 8, fontSize: Platform.OS === 'web' ? 15 : 13 }]} numberOfLines={1}>
                       Tel: {clienteMini.telefono}
                     </Text>
                   ) : null}
                   {clienteMini?.direccion ? (
                     <Text style={[styles.clientNit, { color: C.sub, marginTop: 8, fontSize: Platform.OS === 'web' ? 15 : 13 }]}>
                       {clienteMini.direccion}
                     </Text>
                   ) : null}
                   {Platform.OS === 'web' ? (
              ...
              {ventaEventos.map((ev, idx) => (
                <Text key={idx} style={[styles.note, { color: C.text, marginTop: 6 }]}>
                  <Text style={{ fontWeight: "600" }}>
                    {ev.tipo === "EN_RUTA" ? "Nota en ruta: " : "Nota de entrega: "}
                  </Text>
                  {ev.nota}
                  {!!ev.by ? <Text style={{ color: C.sub }}>{" "}— {ev.by}</Text> : null}
                </Text>
              ))}
...
export default VentaDetallePanel;
