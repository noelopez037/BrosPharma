...
export default function ClienteDetalle() {
  ...
  const uploadPickedAsset = useCallback(
    ...
    [row, empresaActivaId, docPathField, loadCliente]
  );
  ...
  const pickFromCameraOrLibrary = useCallback(
    async (tipo: DocTipo, source: "camera" | "library") => {
      try {
        ...
        Alert.alert("Error", e?.message ?? "No se pudo tomar/elegir la foto");
      }
    },
    [uploadPickedAsset]
  );
  ...
  const pickAndUploadDoc = useCallback(
    ...
    [canEdit, row, docBusyTipo, empresaActivaId, pickFromFiles]
  );
