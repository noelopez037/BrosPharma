-- Retorna cliente_ids con al menos una factura vencida hace más de 60 días y saldo pendiente.
-- Solo considera pagos directos (factura_id IS NOT NULL); es intencional para el bloqueo.
CREATE OR REPLACE FUNCTION public.fn_clientes_bloqueados_cxc(p_empresa_id bigint)
RETURNS TABLE(cliente_id bigint)
LANGUAGE sql SECURITY DEFINER AS $$
  SELECT DISTINCT v.cliente_id
  FROM ventas v
  JOIN ventas_facturas vf ON vf.venta_id = v.id
  LEFT JOIN (
    SELECT factura_id, SUM(monto) AS total_pagado
    FROM ventas_pagos
    WHERE factura_id IS NOT NULL
    GROUP BY factura_id
  ) pf ON pf.factura_id = vf.id
  WHERE v.empresa_id = p_empresa_id
    AND v.estado <> 'ANULADO'
    AND vf.fecha_vencimiento < CURRENT_DATE - INTERVAL '60 days'
    AND vf.monto_total - COALESCE(pf.total_pagado, 0) > 0;
$$;

-- rpc_crear_venta: bloquear ventas a clientes con deuda vencida (excepto ADMIN)
...
begin
  ...
  -- Bloquear ventas a clientes con deuda vencida > 60 días (ADMIN puede omitir)
  if v_role <> 'ADMIN' then
    if exists (
      select 1 from public.fn_clientes_bloqueados_cxc(v_empresa_id)
      where cliente_id = v_cliente_id
    ) then
      raise exception 'CLIENTE_BLOQUEADO_DEUDA_VENCIDA';
    end if;
  end if;
